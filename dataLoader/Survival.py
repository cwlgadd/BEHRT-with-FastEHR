...
class Survival(Dataset):
    def __init__(self, token2idx, label2idx, age2idx, dataframe, max_len, code='code', age='age', label='label', label_time='label_time'):
        ...
        self.label_time = dataframe[label_time]
    ...
    def __getitem__(self, index):
        ...
        label_time = self.label_time[index]
        ...
        return torch.LongTensor(age), torch.LongTensor(code), torch.LongTensor(position), torch.LongTensor(segment), \
               torch.LongTensor(mask), torch.LongTensor([label]), torch.FloatTensor([label_time]), torch.LongTensor([int(patid)])
